class Solution {
public:
    void dfs(vector<vector<int>> &board,int row,int col,vector<vector<bool>>& visited,int n,int m){
        // visited[i][j]=1;
        // board[i][j]='O';
        ...
        for(int l=0;l<4;l++){
            ...
            if(nrow<n && nrow>=0 && ncol<m && ncol>=0 &&  board[nrow][ncol]==1 && !visited[nrow][ncol] ){
               visited[nrow][ncol]=1;
